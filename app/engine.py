...
class FEScraperEngine:
    ...
    def _fetch_wizard_template(self) -> Dict[str, Any]:
        ...
        r = self._request_with_x_appl("GET", wizard_template, verify=get_ca_bundle())
    ...
    def _init_new_portale_session(self) -> None:
        ...
        r = self.session.get(init_url, allow_redirects=False, verify=get_ca_bundle())

        if r.status_code == 501 and r.headers.get("x-red"):
            r = self.session.get(r.headers["x-red"], allow_redirects=False, verify=get_ca_bundle())
        ...
        r = self._request_with_x_appl("GET", perm_url, verify=get_ca_bundle())
        if r.status_code != 200:
        ...
        r = self._request_with_x_appl("GET", vai_url, verify=get_ca_bundle())
        if r.status_code != 200:
        ...
        r = self.session.get(target, allow_redirects=True, verify=get_ca_bundle())
        if r.status_code != 200:
        ...
        r = self._request_with_x_appl("GET", init_light, verify=get_ca_bundle())
        if r.status_code != 200:
    ...
    def login(self, cf: str, pin: str, password: str) -> str:
        ...
        r = self.session.get(login_page, verify=get_ca_bundle())
        if r.status_code != 200:
    ...
    def _wizard_proceed(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        r = self._request_with_x_appl("POST", procedi_url, json=payload, headers=headers, verify=get_ca_bundle())
        if r.status_code != 200:
    ...
    def _wizard_set_user_choice(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        r = self._request_with_x_appl("POST", scelta_url, json=payload, headers=headers, verify=get_ca_bundle())
        if r.status_code != 200:
    ...
    def get_b2b_tokens(self):
        ...
        self.session.get(
            f"https://ivaservizi.agenziaentrate.gov.it/cons/cons-web/?v={unix_ms()}",
            verify=get_ca_bundle(),
        )
        ...
        self.logger(f"Token B2B response: HTTP {r.status_code}")
        if r.status_code != 200:
            raise Exception(f"Impossibile ottenere Token B2B (HTTP {r.status_code}).")
    ...
    def fetch_invoices(self, start_str: str, end_str: str, category: str) -> Dict[str, Any]:
        ...
        for d_dal, d_al in chunks:
            ...
            if category == "RICEVUTE":
                url = (
                    "https://ivaservizi.agenziaentrate.gov.it/cons/cons-services/rs/fe/"
                    f"ricevute/dal/{dal_c}/al/{al_c}/ricerca/ricezione?v={unix_ms()}"
                )
            else:
                url = (
                    "https://ivaservizi.agenziaentrate.gov.it/cons/cons-services/rs/fe/"
                    f"emesse/dal/{dal_c}/al/{al_c}?v={unix_ms()}"
                )
            ...
            if category == "EMESSE" and r.status_code == 404:
                url = (
                    "https://ivaservizi.agenziaentrate.gov.it/cons/cons-services/rs/fe/"
                    f"emesse/dal/{dal_c}/al/{al_c}/ricerca/invio?v={unix_ms()}"
                )
                r = self.session.get(url, headers=self.headers_cons)
